...
При локально запущенном Tomcat, адрес запроса http://localhost:8080/hello/contacts". 

такой и пишешь полный с локалхостПример запроса http://localhost:8080/hello/contacts?nameFilter=^A.$ Принимает обязательный параметр nameFilter, который содержит регулярное выражение, по которому необходимо отсортировать контакты.
